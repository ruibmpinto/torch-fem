...
class FEM(ABC):
    ...
    def surrogate_integrate_material(
        ...
        This function builds graphs from displacements and uses the GraphOrge
        surrogate model to predict forces and compute stiffness matrices.
        
        Parameters
        ----------
        model : GNNEPDBaseModel
            Pre-loaded Graphorge material patch model
        u : Tensor
            Displacement history tensor of shape (n_increments, n_nodes, n_dim)
        n : int
            Current increment number
        du: Tensor
            Displacement increment tensor of shape (, n_dim)
        is_time_series : bool, default=False
            Whether to use time series data
        patch_data : dict, default=None
            Additional patch data (unused in current implementation)
            
        Returns
        -------
        k : Tensor
            Element stiffness matrices of shape: (n_elem, n_dof_elem,
            n_dof_elem)
        f : Tensor
            Element internal force vectors of shape (n_elem, n_dof_elem)
        """
        # Compute updated configuration
        ...
        for idx_elem in range(self.n_elem):
            ...
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            # Compute stiffness matrix within Graphorge
            # which handles the full graph reconstruction from displacements
            # for single material patches, i.e., no batching
            ...
            #        9730.2079,  13379.0358, -30406.8996,  68111.4551]])
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            # Forward pass through model to get forces
    ...
    def _load_Graphorge_model(self, model_directory: str, 
        ...
        ----------
        model_directory : str
            Path to the directory containing the trained Graphorge model
        device_type : str, default='cpu'
            Device type for model execution ('cpu' or 'cuda')
            
        Returns
        -------
        model : GNNEPDBaseModel
            Loaded and configured Graphorge model ready for inference
    ...
    def solve(
        ...
        for n in range(1, N):
            ...
            for i in range(max_iter):
                ...
                # indicator[idk_element] = 1.0; matpatch_id
                # if is_mat_patch:
    ...
    def solve_matpatch(
        ...
        Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        """Solve the FEM problem with material patches using surrogate 
        integration.

        Args:
            is_mat_patch (Tensor): Element-wise flag indicating material 
            patch usage.
                Shape: (n_elem,). Values: 0 = use standard integration, 
                >0 = material patch ID for surrogate integration.
            increments (Tensor): Load increment stepping.
            max_iter (int): Maximum number of iterations during Newton-Raphson.
            rtol (float): Relative tolerance for Newton-Raphson convergence.
            atol (float): Absolute tolerance for Newton-Raphson convergence.
            stol (float): Solver tolerance for iterative methods.
            verbose (bool): Print iteration information.
            method (str): Method for linear solve ('spsolve','minres','cg',
            'pardiso').
            device (str): Device to run the linear solve on.
            return_intermediate (bool): Return intermediate values if True.
            aggregate_integration_points (bool): Aggregate integration points 
            if True.
            use_cached_solve (bool): Use cached solve.
            nlgeom (bool): Use nonlinear geometry if True.
            return_volumes (bool): Return element volumes for each increment
            if True.
        ...
        Returns:
                ...
                internal forces, stress, deformation gradient, and 
                    material state.
                    If return_volumes=True, also returns element volumes 
                        with shape
                    (num_increments, num_elem, 1).
        ...
        if torch.any(patch_mask):
            # Load Graphorge model when material patches are present
            # You can specify the model directory path here
            model = self._load_Graphorge_model(
                model_directory=(
                    '/Users/rbarreira/Desktop/machine_learning/'
                    'material_patches/graphorge_material_patches/src/'
                    'graphorge/projects/material_patches/elastoplastic/2d/quad4/'
                    'mesh1x1/ninc100/stiffness_prediction_rnn/reference/3_model'),
                ...
                if torch.any(patch_mask):
                    ...
                    for patch_id in patch_ids:
                        ...
                        if patch_indices.numel() > 0:
                            ...
                            k_surr, f_surr = self.surrogate_integrate_material(
                                model, u, n, du, is_time_series=False)
