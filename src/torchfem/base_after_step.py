...
class FEM(ABC):
    ...
    def surrogate_integrate_material(
        ...
        Args:
            ...
            is_time_series (bool): Legacy parameter, currently unused. 
                Defaults to False.
        ...
        for idx_elem in range(self.n_elem):
            ...
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            # Single-step graph construction (current coordinates and displacements only)
            current_coords = (elem_coords_ref + elem_u_current).detach().numpy()
            current_disps = elem_u_current.detach().numpy()
            
            # Set up graph edges
            connect_radius = 4 * np.sqrt(np.sum([x**2 for x in 
                get_elem_size_dims([1.0, 1.0], [1, 1], dim)]))
    
            # Create mesh connectivity for single element
            ...
            # Set up edges
            bd_node_indices = list(range(self.etype.nodes))
            ...
            print(f'edges_indexes_mesh: {edges_indexes_mesh}')
            print(f'boundary_node_set: {boundary_node_set}')
            
            print(f'connected_nodes_all: {connected_nodes_all}')

            gnn_patch_data.set_graph_edges_indexes(
                connect_radius=connect_radius,
                edges_indexes_mesh=edges_indexes_mesh)
            
            # Set node and edge features for current step
            node_features_matrix = np.concatenate([
                current_coords,  # Shape: [n_nodes, n_dim]  
                current_disps    # Shape: [n_nodes, n_dim]
            ], axis=1)  # Result: [n_nodes, 2*n_dim]
            
            # Set edge features (distances and relative displacements)
            edges_indexes = gnn_patch_data.get_graph_edges_indexes()
            n_edges = edges_indexes.shape[1]
            edge_features_matrix = np.zeros((n_edges, 2*dim))
            
            for i in range(n_edges):
                node1_idx, node2_idx = edges_indexes[:, i]
                # Edge vector (distance)
                edge_vector = current_coords[node2_idx] - current_coords[node1_idx]
                # Relative displacement
                relative_disp = current_disps[node2_idx] - current_disps[node1_idx]
                edge_features_matrix[i] = np.concatenate([edge_vector, relative_disp])
            
            # Set features in GraphData
            ...
            # Get input features using standard method
    ...
    def solve_matpatch(
        ...
        Args:
            ...
            is_time_series (bool): Whether the surrogate model uses time 
                series (currently unused). Defaults to True.
        ...
        if torch.any(patch_mask):
            ...
            if model_directory is None:
                model_directory = (
                    ...
                    'graphorge/projects/material_patches/elastoplastic/'
                    '2d/quad4/mesh1x1/ninc100/stiffness_prediction_rnn/'
                    'reference/3_model')
